document.addEventListener('DOMContentLoaded', function () {
  ...
  const bodyData = {
    type: "construction" // Dữ liệu bạn muốn gửi trong yêu cầu POST
  };
  // Gọi API
  fetch(`${apiUrl}`, {
    method: 'POST',
    ...
    .then(data => {
      ...
      const apiDataDiv = document.getElementById('add-construction-real');
      if (data?.data && Array.isArray(data?.data?.postings)) {
        ...
        links.forEach(link => {
          link.addEventListener('click', async function (event) {
            ...
            localStorage.setItem('selected_item_construction_real_mobile', itemData);
            window.location.href = 'cong-trinh-thuc-te-item.html';
